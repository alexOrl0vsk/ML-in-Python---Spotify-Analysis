...
def singles_or_colabs(Singles):
	...
	# input :: Singles (bool) 	-> True return only single "artist_count" entries 
	#							-> False return only colab songs 
	df = loadData(False)
	X = df.drop(columns=['streams','key','mode'])
...
def trySizesMPL():
	...
	df_temp.to_csv("results_csv/trySizesMPL.csv")

def gridSeach_PCA(caseID,scaler,model):
	# general pipeline with different (scalers / no. of PCA components / models) with same step names 
	...
	df_temp = pd.DataFrame(grid.cv_results_)
	df_temp = df_temp.sort_values("rank_test_r2")
	df_temp.to_csv("results_csv/gridSeach_PCA.csv")
...
def main():
	#gridSeach_PCA(1,RobustScaler(),MLPRegressor(hidden_layer_sizes=(15,15,10), random_state=1,max_iter=10000, solver='adam'))
	bestConfigs(1)
